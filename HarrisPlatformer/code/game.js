...
// Oil is initialized based on the character, but otherwise has a
// size and position
function Oil(pos, ch) {
  this.pos = pos;
  ...
  if (ch == "=") {
    ...
    // Drip oil. Repeat back to this pos.
    this.speed = new Vector(0, 3);
    this.repeatPos = pos;
  }
}
Oil.prototype.type = "oil";
...
Lazer.prototype.act = function(step) {
  var newPos = this.pos.plus(this.speed.times(step));
  this.pos = newPos
};

Oil.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle oil by calling playerTouched
  if (obstacle)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches oil and the player hasn't won
  // Player loses
  if (type == "oil" && this.status == null) {
    this.status = "lost";
    this.finishDelay = .3;
  } else if (type == "lazer") {
    this.status = "lost";
